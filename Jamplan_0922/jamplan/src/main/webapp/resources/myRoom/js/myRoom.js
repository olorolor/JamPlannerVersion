...
// 로드 되면 출력한다.
	...
	// aria-expanded가 한번 눌려서 true가 되면 다시 false로 돌아가지 않아서 그걸 고치기위한 부분
	/*console.log('show 클래스 들어가기 직전');
	$('.collapse').on('click', function () {
		if($(this).attr('class') == 'show') {
			console.log('show 클래스 들어옴');
			$(this).attr("");
		}
	})*/
	...
	inputS.addEventListener("keyup", function(event) {
	    event.preventDefault();
	    if (event.keyCode === 13) {
	    	console.log("keyup이벤트 들어왔나?");
	        document.getElementById("searchTeam").submit();
	    };
	});*/
...
function ajaxGetTeamList() {
	...
	$.ajax({
			...
			success : function(data){ 
					$.each(data,function(index, item) {
					var teamList = '';
					teamList += '<button class="btn btn-outline-light text-dark" id="myTeam'
							+ index + '" data-toggle="collapse" data-target="#myPlan'+ index 
							+ '">'+ item.teamName + '</button>';
					teamList += '<div id="myPlan' + index + '" class ="collapse">';
					
					let RetrunList = teamList;
					let teamN =  item.teamName;
					let i = index;
					...
					$.ajax({	
						...
						success : function(data) {
							...
							/*teamList += '<div id="myPlan' + i + '" class ="collapse planlist" >';*/
							$.each(data,function(index,item){
								//console.log(item.teamName);
								teamList += '<p class = "move" value = "'+ item.planNo + '">'+ item.planName +'</p>';
							})
							teamList += '</div>';
						}
					})
...
/*function ajaxGetPlanList(id, teamNo) {
	let planList = '';
	...
	$.ajax({	
		...
		success : function(data) {
			//console.log("ajax get teamName"+teamName);
			planList += '<div id="myPlan' + teamNo + ' class ="collapse planlist" >';
			$.each(data,function(index,item){
				//console.log(item.teamName);
				planList += '<p class = "move" value = '+item.planNo+'>'+ item.planName +'</p>';
			})
			planList += '</div>';
		}
	})
